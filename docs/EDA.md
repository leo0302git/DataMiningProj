...
| 问题 | 数据支撑与图表表现 | 可能影响 | 后续处理 |
|---|---|---|---|
| 597 个样本对应 196 个描述符 | 来自原始数据结构和 [特征统计表](../results/eda/dia/feature_summary.csv)，样本数约为特征数的 3 倍；这是维度统计，没有单独作图 | 高维小样本容易过拟合，验证结果波动大 | 使用固定的分层 K 折；报告各折均值和波动 |
| 阳性只占 24.8% | [类别分布图](../results/eda/dia/class_distribution.png) 中阴性柱为 449（75.2%），阳性柱为 148（24.8%），高度约为 3:1 | Accuracy 可能掩盖少数类效果 | 使用分层划分，并报告 F1、ROC-AUC、PR-AUC 等分类指标；类别权重作为待比较选项 |
| 17 个常量特征、58 个极稀疏特征 | [特征统计表](../results/eda/dia/feature_summary.csv) 中 17 列 `std=0`，58 列 `zero_fraction>=0.95`；[箱线图](../results/eda/dia/boxplots.png) 和 [直方图](../results/eda/dia/histograms.png) 中多个片段特征集中在 0，仅有少量非零点 | 无信息特征增加参数和规则搜索空间 | 在每个训练折删除零方差特征；近零方差特征是否删除由验证结果决定 |
| 特征尺度和长尾差异大 | [直方图](../results/eda/dia/histograms.png) 中 `Ipc`、`Kappa3` 等主体挤在低值端；[分位数图](../results/eda/dia/quantile_plots.png) 在最高分位处突然上升，`Ipc` 最大值约为 \(3.85\times10^{14}\) | 线性/神经网络训练不稳定，等宽分箱失真 | 线性模型在训练折做标准化；RRL 比较分位数或监督离散化；树模型保留原尺度 |
| 101 对高度相关描述符 | [相关性图](../results/eda/dia/correlation.png) 出现成块的深色区域；[散点图](../results/eda/dia/scatterplots.png) 中前三组点几乎落在直线上，均显示 \(|r|=1.000\) | 线性系数不稳定、规则重复、解释冗余 | 在训练折内做相关性约简，或保留给树模型并在解释时标明冗余 |
| 单特征与标签关系较弱 | 单特征与 `Label` 的最大 \(|r|\) 仅 0.216；[箱线图](../results/eda/dia/boxplots.png) 中两类箱体和散点明显重叠，[直方图](../results/eda/dia/histograms.png) 中两类分布也大量重合 | 单变量阈值难以获得高性能 | 需要组合特征或非线性交互，同时严格控制复杂度 |
...
| 问题 | 数据支撑与图表表现 | 可能影响 | 后续处理 |
|---|---|---|---|
| `Sex` 是无序类别且各组分布不同 | [目标与类别分布图](../results/eda/abalone/target_distribution.png) 显示三组数量接近；[分组箱线图](../results/eda/abalone/boxplots.png) 中幼体 I 的尺寸、重量和 `Rings` 整体低于 M/F，I、F、M 的 `Rings` 均值分别为 7.890、11.129、10.705 | 直接使用 M/F/I 字符或 1/2/3 会引入错误关系 | 在训练流程中使用 One-Hot Encoding |
| 极端/矛盾测量值 | [箱线图](../results/eda/abalone/boxplots.png) 中 `Height=1.130` 远离箱体；[分位数图](../results/eda/abalone/quantile_plots.png) 的 `Height` 曲线末端突跃，[散点图](../results/eda/abalone/scatterplots.png) 中也出现孤立高度点；原始数据另有 4 行 `Shucked weight > Whole weight` | 对均方误差、标准化和线性回归斜率影响较大 | 首轮保留并建立异常标记；再做“保留/修正或剔除”的敏感性比较，不直接批量删除 |
| 重量和目标右偏、高龄样本稀少 | [直方图](../results/eda/abalone/histograms.png) 中重量和 `Rings` 有明显右尾；[目标分布图](../results/eda/abalone/target_distribution.png) 在 9–10 环附近最高，20 环后样本很少；`Rings>=20` 仅 62 个 | 模型容易低估高龄鲍鱼，RMSE 对尾部误差敏感 | 除 RMSE 外报告 MAE 和 \(R^2\)，并单独检查高环数样本误差 |
| 输入特征高度相关 | [相关性图](../results/eda/abalone/correlation.png) 中长度、直径和重量区域大面积深红；`Length`/`Diameter` 的 \(r=0.987\)，`Whole weight`/`Shucked weight` 的 \(r=0.969\) | 线性系数可能不稳定，重复信息增加模型复杂度 | 线性模型使用正则化；树模型保留原特征；不默认使用 PCA |
| 特征尺度不同 | [特征统计表](../results/eda/abalone/feature_summary.csv) 显示长度约 0.075–0.815、整体重量约 0.002–2.826、`Rings` 为 1–29；各 [直方图](../results/eda/abalone/histograms.png) 的横轴范围明显不同 | 影响线性、距离和神经网络类模型 | 在训练折内标准化连续特征；树模型不强制标准化 |
| 尺寸与 `Rings` 关系不是纯线性 | [散点图](../results/eda/abalone/scatterplots.png) 虽有上升趋势，但同一尺寸对应的 `Rings` 范围很宽，并呈现性别/年龄组聚集；单特征相关系数仅约 0.421–0.628 | 单一线性模型可能欠拟合 | 保留线性模型作为基线，并与树集成和 RRL 回归版本比较 |
